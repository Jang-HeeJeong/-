...
1. **[Vercel 스타일 레퍼런스](#vercel--style-reference)** — 채택한 디자인 시스템 원문
2. **[이 앱에 적용할 때](#이-앱에-적용할-때)** — 검토 툴에 맞게 조정한 지침 ⚠ **여기를 꼭 읽으세요**

원문은 마케팅 페이지용으로 만들어진 시스템이라, 조밀한 데이터 입력 화면인 이 툴에는
그대로 쓸 수 없는 부분이 있습니다. 2번에 무엇을 어떻게 바꿔 쓰는지 정리했습니다.
...
위 시스템은 **마케팅 페이지용**입니다. 이 툴은 28행 × 8열짜리 숫자 입력표와
12열짜리 결과표가 핵심인 **조밀한 업무 화면**이라, 그대로 옮기면 못 쓰게 됩니다.

무엇을 그대로 쓰고, 무엇을 바꿔 쓰는지 정리합니다.

## 그대로 가져가는 것

| 항목 | 이유 |
|---|---|
| 색 팔레트 전체 (`#fafafa` / `#171717` / `#ebebeb` …) | 그대로 사용 |
| **헤어라인 테두리로 깊이 표현** (그림자 금지) | 조밀한 표에 그림자는 시야를 흐림. 오히려 더 잘 맞음 |
| **6px 모서리** (칩·버튼은 필요 시 pill) | 그대로 |
| **Geist Mono = 라벨·메타데이터** | 숫자표에 최적. 자릿수가 맞아 비교가 쉬움 |
| 4px 그리드 | 그대로 |
| ▲ 기호 = 접기/펼치기 표시 | 원문에도 "interactive collapse marker" 용도가 있음. 그룹 접기에 사용 |

## 바꿔 쓰는 것

### 1. 타입 스케일 — 큰 사이즈는 안 씁니다

원문의 30 / 56 / 64px은 히어로 헤드라인용입니다. 이 툴에는 히어로가 없습니다.

| 원문 | 이 앱에서 |
|---|---|
| display 64px, heading-lg 56px | **쓰지 않음** |
| heading 30px | 결과 패널의 "전체 판정"에만 (그것도 20~24px로 축소) |
| body 16px | 설명문·안내문 |
| caption 13px | 폼 라벨, 버튼 |
| eyebrow 11px | 섹션 제목, 표 헤더 |
| — | **표 셀 12px 추가** (원문에 없음. 조밀한 표에 필요) |

### 2. 섹션 간격 — 96~128px은 너무 큽니다

이 툴은 한 화면에 입력표와 결과를 같이 봐야 합니다. **섹션 간격은 16~32px**을 씁니다.
원문의 `--spacing-208`은 쓰지 않습니다.

### 3. 색으로 구분하던 것을 명도·서체로 바꿉니다 ⚠

원문은 "0% colorfulness"가 규칙입니다. 그런데 이 툴에서 색은 장식이 아니라 **의미**를 담고 있습니다.

현재: 🔵 파랑 = 표제기 기준 / 🟢 초록 = 제품 함량 — 이걸 없애면 **"기준과 제품 값이 구분 안 된다"는 원래 문제로 돌아갑니다.**

**해결:** 색조(hue) 대신 **명도 차이 + 서체 + 그룹 헤더**로 구분합니다. 원문의
"depth through tonal value shifts" 원칙에 부합합니다.

| | 배경 | 헤더 |
|---|---|---|
| 표제기 기준 | `#f4f4f4` (한 단계 어둡게) | `#171717` 바탕에 흰 글씨 |
| 제품 함량 | `#ffffff` (밝게) | `#ebebeb` 바탕에 검은 글씨 |

값 자체는 Geist Mono로 통일해 자릿수를 맞춥니다.

### 4. 부적합 표시는 빨강을 유지합니다 ⚠ 의도적 위반

원문은 Terminal Green(`#297a3a`) 외의 유채색을 금지합니다.
하지만 **이건 허가사항 검토 도구**입니다. 부적합을 놓치면 실무 사고로 이어집니다.

**결정: 부적합에는 빨강을 씁니다.** 스타일 순수성보다 안전이 우선입니다.

| 상태 | 색 | 표기 |
|---|---|---|
| 적합 | `#297a3a` (Terminal Green) | `✓ 적합` — 원문 허용 범위 |
| **부적합** | `#c62828` | `✕ 부적합` — **의도적 확장** |
| 확인불가 | `#8f8f8f` (Graphite) | `— 확인불가` |

색맹 사용자를 위해 색만으로 구분하지 않고 **기호(✓ / ✕)를 항상 함께** 표시합니다.

### 5. 입력칸은 노란 음영 ⚠ 의도적 위반

원문은 유채색을 금지하지만, 이 툴에서 **작업자가 값을 넣어야 하는 칸**은
28~50행 중 몇 개뿐입니다. 무채색으로는 어디를 채워야 하는지 드러나지 않습니다.

| | 색 |
|---|---|
| 입력칸 배경 | `#fff8db` |
| 입력칸 테두리 | `#e8d071` |
| 입력 열 전체 | `#fffdf0` (아주 옅게) |
| 헤더에 `✎` 표시 | `#fdf3c4` |

### 6. 숫자도 본문과 같은 서체 ⚠ 원문 규칙과 다름

원문은 "라벨·숫자는 반드시 Mono"라고 못박습니다. 그런데 Geist Mono의 `0`이
지나치게 눈에 띄어 숫자를 읽는 데 방해가 됐습니다.

**결정: 숫자도 Geist/Pretendard를 쓰고, 자릿수 정렬은 `tabular-nums`로 유지합니다.**
Mono를 쓰지 않아도 자릿수는 맞습니다.

### 7. 검정을 한 단계 옅게

`#171717`을 넓은 면(표 헤더 밴드)에 쓰니 화면이 무거웠습니다.
글자에는 그대로 쓰되 **면에는 옅은 값**을 씁니다.

| 용도 | 원문 | 이 앱 |
|---|---|---|
| 기준 헤더 바탕 | `#171717` | `#575757` |
| 기준 하위 헤더 | — | `#6e6e6e` |
| 기본 버튼 | `#171717` | `#2e2e2e` |

### 8. 적합/부적합은 칩으로 강하게

글자색만으로는 조밀한 표에서 눈에 안 들어옵니다.

| 상태 | 표시 |
|---|---|
| 적합 | `✓ 적합` — 연녹색 배경 칩 |
| **부적합** | `✕ 부적합` — **빨강 채운 칩 + 흰 글씨**, 행 전체 붉은 배경, 왼쪽 3px 띠 |
| 미입력 | `— 미입력` — 회색 칩 |

### 9. 폰트 로딩

**Geist와 Geist Mono는 Google Fonts에 등재돼 있어 그대로 쓸 수 있습니다.**
한글은 Pretendard(jsDelivr)가 이어받습니다.

```css
--sans: 'Geist', 'Pretendard Variable', 'Pretendard', system-ui, sans-serif;
--mono: var(--sans);   /* 6번 참고 — 숫자도 본문 서체를 쓴다 */
```

**한글이 대부분인 화면**이라 한글 폰트(Pretendard, Malgun Gothic)가 실제로는 더 자주 쓰입니다.
Geist의 tracking 값(-0.06em 등)은 한글에서 글자가 붙어 보이므로 **한글 텍스트에는 적용하지 않습니다.**

## 요약 — 이 앱의 규칙

```
바탕      #fafafa
카드      #ffffff + 헤어라인 테두리 (그림자 없음)
글자      #171717 (본문) / #4d4d4d (보조) / #8f8f8f (흐림)
숫자·라벨  Geist Mono
본문      Geist Sans → 한글은 Pretendard
모서리    6px
간격      4px 배수, 섹션 간 16~32px
색        기준/제품은 명도로, 판정만 초록·빨강, 입력칸만 노랑
검정      면에는 #575757 / 글자에만 #171717
```

## 색·서체를 바꾸고 싶을 때

이 문서의 규칙은 **`app/디자인.css` 한 파일**에 들어 있습니다.
`index.html`(실제 쓰는 화면)과 `new.html`(디자인 실험용)이 같은 파일을 읽으므로,
여기만 고치면 두 화면에 함께 반영됩니다.

```
app/디자인.css   ← 색·서체·모서리·판정 칩 정의 (:root 의 --paper, --input-bg 등)
```

예를 들어 입력칸 노란색이 너무 진하면 `--input-bg` 값 하나만 바꾸면 됩니다.

### 딱 한 군데 예외 — 워드 파일

`app/판정_규칙.js`의 워드 파일 생성 부분(`generateFullWordDoc` 등)에는
`#1a4b8c` 같은 색 코드가 그대로 남아 있습니다.
**워드는 CSS 변수(`var(--…)`)를 읽지 못하므로 일부러 바꾸지 않았습니다.**
워드 문서의 색을 바꾸려면 그 부분의 색 코드를 직접 고쳐야 합니다.

## 바뀐 규칙 — 기준 vs 제품 구분 (현장 의견 반영)

처음에는 원문 지침대로 **회색 명도만으로** 표제기 기준과 제품 함량을 갈랐습니다.
실제로 써 보니 **회색 농도 차이가 눈에 안 들어온다**는 의견이 나와 바꿨습니다.

```
표제기 기준   옅은 남색   머리글 #dbe4f0 / 글자 #1e3a5f / 본문칸 #f5f8fc
제품 함량     흰 바탕     머리글 #f0f0f0 / 글자 #3d3d3d / 본문칸 #ffffff
두 무리 사이  2px 세로선  #b9cbe2  ← 어디까지가 기준인지 선으로 끊는다
```

색조를 하나(남색)만 더 썼습니다. 초록·빨강은 판정, 노랑은 입력 전용이라 겹치지 않습니다.
값은 `app/디자인.css`의 `--std-head`, `--std-bg`, `--std-rule` 등입니다.

**2단 머리글의 밑선**은 묶음 칸(표제기 기준·제품 함량) 아래에만 한 줄이 더 생겨
선 높이가 어긋나 보였습니다. `th[rowspan]`과 아래 단에만 선을 주어 한 줄로 이어지게 했습니다.
